...
app.get('/whois', (req, res) => {
  res.status(200).render('whois.ejs')
})
...
app.get('/currency-converter', (req, res) => {
  res.status(200).render('currency-converter.ejs')
});
...
app.get("/chuck-norris",function (req,res){
res.status(200).render('chuck-norris.ejs');
});

app.get('/language-detector',(req,res)=>{
  res.status(200).render('language-detector.ejs');
});
...
app.post('/language-detector', async (req,res)=>{
  (async () => {
    await request.get(
      {
        url: 'https://api.api-ninjas.com/v1/textlanguage?text=' + req.body.lang.toLowerCase(),
        headers: {
          'X-Api-Key': 'pKn873OBShoVD7iS9U1I9Q==WS8rZnDXhtDOGIC3'
        }
      },
...
app.get("/loader",(req,res)=>res.render('loader.ejs'))
